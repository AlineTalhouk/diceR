...
#'   different algorithms. Matrix can be flattened already
#' @param is.relabelled TRUE/FALSE defaults to TRUE, but if FALSE the data will
#'   be relabelled
#' @param is.flat set to TRUE to indicate that impute matrix is already 2D with
#'   rows as cases and columns as clusterings
#' @return a vector of cluster assignment based on majority vote
#' @author Aline Talhouk
#' @export
#' @examples
#' data(E_imputed)
#' table(majority_voting(E_imputed, is.relabelled = FALSE, is.flat = FALSE))
