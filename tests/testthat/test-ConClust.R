...
test_that("No method given means all methods", {
  x1 <- ConClust(dat, nk = 2:4, reps = 1, progress = FALSE)
  expect_error(x1, NA)
})

test_that("Output can be saved with or without time in file name", {
  x1 <- ConClust(dat, nk = 2:4, reps = 10, method = "hcAEucl", progress = FALSE,
                 save = TRUE)
  x2 <- ConClust(dat, nk = 2:4, reps = 10, method = "hcAEucl", progress = FALSE,
