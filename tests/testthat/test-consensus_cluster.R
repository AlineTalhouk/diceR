...
test_that("No algorithms given means all algorithmss", {
  x1 <- consensus_cluster(dat, nk = 2:4, reps = 1, progress = FALSE)
  expect_error(x1, NA)
})

test_that("Output can be saved with or without time in file name", {
  x1 <- consensus_cluster(dat, nk = 2:4, reps = 10, algorithms = "hcAEucl",
                          progress = FALSE, save = TRUE)
  x2 <- consensus_cluster(dat, nk = 2:4, reps = 10, algorithms = "hcAEucl",
