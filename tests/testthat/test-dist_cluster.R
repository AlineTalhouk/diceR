...
test_that("Check dist_cluster with E_LCE", {
  ...
  tree_complete_clustered <-
    dist_cluster(Z = tree_complete, maxclust = K)
  ...
  expect_equal(sum(sort(unique(
    tree_single_clustered
  )) != 1:K), 0)
  expect_equal(sum(sort(unique(
    tree_complete_clustered
  )) != 1:K), 0)
  expect_equal(sum(sort(unique(
    tree_average_clustered
  )) != 1:K), 0)
})
...
test_that("Check dist_cluster with a very small tree",{
  ...
  expect_equal(sum(sort(unique(dist_cluster(Z=tree_short,maxclust = K)))!=1:K),0)
