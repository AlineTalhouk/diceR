...
test_that("Check relabelCl for relabelling and total number of classes", {
...
test_that("Check relabelCl with negative class",{
  mat_negClass <- mat
  mat_negClass[2, 3] <- -1
  expect_error(relabelCl(mat_negClass))
})

test_that("Check relabelCl with wrong inputs",{
  expect_error(relabelCl(3))
  expect_error(relabelCl(c(1, 2)))
  expect_error(relabelCl(data.frame(
