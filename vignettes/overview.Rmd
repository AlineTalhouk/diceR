...
The list of internal validity indices used for evaluating clusters are:
  ...
  - Calinski-Harabasz, Gamma, C-index, Davies-Bouldin, and Rousseeuw's Silhouette from the `clusterSim` R package
  - Dunn index and Connectivity measure from the `clValid` R package
  - SD and S_Dbw adapted from the archived `clusterCrit` R package
